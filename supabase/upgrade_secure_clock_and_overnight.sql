...
begin
  ...
  v_is_regular_workday := extract(isodow from v_work_date)::smallint = any(v_settings.work_days)
    and not exists (select 1 from public.holidays where org_id = v_user.org_id and holiday_date = v_work_date);
  ...
  if p_action = 'clock_in' then
    ...
    insert into public.attendance_records (
      ...
      coalesce(p_note,''), 0, 0, 'none', 0, 'none', v_user.org_id, v_shift.id,
