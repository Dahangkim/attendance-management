...
const approvedLeaveForRecord = (record: AttendanceRecord, requests: CorrectionRequest[]) => requests.find((request) => request.status === "approved" && request.employee_id === record.employee_id && ["annual_leave", "comp_time", "sick_leave", "special_leave", "other_leave"].includes(request.request_type) && request.target_date <= record.work_date && (request.end_date || request.target_date) >= record.work_date);
