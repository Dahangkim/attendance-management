...
const approvedLeavesForRecord = (record: AttendanceRecord, requests: CorrectionRequest[]) => requests.filter((request) => request.status === "approved" && request.employee_id === record.employee_id && ["annual_leave", "comp_time", "sick_leave", "special_leave", "other_leave"].includes(request.request_type) && request.target_date <= record.work_date && (request.end_date || request.target_date) >= record.work_date);
const approvedLeaveForRecord = (record: AttendanceRecord, requests: CorrectionRequest[]) => approvedLeavesForRecord(record, requests)[0];
const attendanceStatusLabel = (record: AttendanceRecord, requests: CorrectionRequest[]) => {
  if (!record.clock_in_at) {
    const leave = approvedLeaveForRecord(record, requests);
...
const attendanceStatusTone = (record: AttendanceRecord, requests: CorrectionRequest[]) => !record.clock_in_at && (approvedLeaveForRecord(record, requests) || approvedEmergencyMinutesForRecord(record, requests) > 0 || [0, 6].includes(new Date(`${record.work_date}T12:00:00+09:00`).getDay())) ? "positive" : statusTone(record.attendance_status);
...
const attendanceLeaveLabel = (record: AttendanceRecord, requests: CorrectionRequest[]) => {
  const labels = approvedLeavesForRecord(record, requests).map((request) => {
