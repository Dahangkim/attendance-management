...
const leaveDayLabel = (minutes: number) => `${Number((minutes / 480).toFixed(3))}일`;
const leaveSubtypePlaceholder = (requestType: string) => requestType === "special_leave" ? "예: 본인 결혼, 가족 장례" : "예: 장기재직휴가, 기관 자체휴가";
const requestValueLabel = (request: CorrectionRequest) => {
  ...
  if (["special_leave", "other_leave"].includes(request.request_type)) return `${request.request_subtype || (request.request_type === "special_leave" ? "특별휴가" : "교육휴가")}, ${leaveDayLabel(minutes)}`;
...
const approvedEmergencyRequestsWithoutAttendance = (records: AttendanceRecord[], requests: CorrectionRequest[]) => requests.filter((request) => request.request_type === "emergency_support" && request.status === "approved" && !records.some((record) => record.employee_id === request.employee_id && (record.id === request.attendance_record_id || record.work_date === request.target_date)));
...
const overtimeBreakdownLabel = (regularMinutes: number, emergencyMinutes: number) => {
  const parts = [regularMinutes > 0 ? `시간외근무 ${formatMinutes(regularMinutes)}` : "", emergencyMinutes > 0 ? `긴급지원 ${formatMinutes(emergencyMinutes)}` : ""].filter(Boolean);
  if (parts.length > 1) parts.push(`합계 ${formatMinutes(regularMinutes + emergencyMinutes)}`);
  return parts.join(", ");
};
const weekRange = (dateKey: string) => {
  const date = new Date(`${dateKey}T12:00:00+09:00`);
  const mondayOffset = (date.getDay() + 6) % 7;
  const monday = new Date(date.getTime() - mondayOffset * 86_400_000);
  const sunday = new Date(monday.getTime() + 6 * 86_400_000);
  return { start: KST_DATE.format(monday), end: KST_DATE.format(sunday) };
};
...
const emergencyStartAt = (request: CorrectionRequest) => request.start_time ? `${request.target_date}T${request.start_time.slice(0, 8)}+09:00` : null;
const emergencyEndAt = (request: CorrectionRequest) => request.end_date && request.end_time ? `${request.end_date}T${request.end_time.slice(0, 8)}+09:00` : null;
const effectiveClockInAt = (record: AttendanceRecord, requests: CorrectionRequest[]) => {
  const candidates = [record.clock_in_at, ...approvedEmergencyRequestsForRecord(record, requests).map(emergencyStartAt)].filter(Boolean) as string[];
  return candidates.sort((left, right) => new Date(left).getTime() - new Date(right).getTime())[0] || null;
};
const effectiveClockOutAt = (record: AttendanceRecord, requests: CorrectionRequest[]) => {
  const candidates = [record.clock_out_at, ...approvedEmergencyRequestsForRecord(record, requests).map(emergencyEndAt)].filter(Boolean) as string[];
  return candidates.sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0] || null;
};
const approvedLeavesForRecord = (record: AttendanceRecord, requests: CorrectionRequest[], holidays: Holiday[] = []) => {
  const date = new Date(`${record.work_date}T12:00:00+09:00`);
  if ([0, 6].includes(date.getDay()) || holidays.some((holiday) => holiday.holiday_date === record.work_date)) return [];
  return requests.filter((request) => request.status === "approved" && request.employee_id === record.employee_id && ["annual_leave", "comp_time", "sick_leave", "special_leave", "other_leave"].includes(request.request_type) && request.target_date <= record.work_date && (request.end_date || request.target_date) >= record.work_date);
};
const approvedLeaveForRecord = (record: AttendanceRecord, requests: CorrectionRequest[], holidays: Holiday[] = []) => approvedLeavesForRecord(record, requests, holidays)[0];
const attendanceStatusLabel = (record: AttendanceRecord, requests: CorrectionRequest[], holidays: Holiday[] = []) => {
  if (!record.clock_in_at) {
    const leave = approvedLeaveForRecord(record, requests, holidays);
    if (leave) return REQUEST_TYPE_LABEL[leave.request_type] || "승인 휴가";
    if (approvedEmergencyMinutesForRecord(record, requests) > 0) return [0, 6].includes(new Date(`${record.work_date}T12:00:00+09:00`).getDay()) ? "휴일 긴급지원" : "긴급지원 근무";
    if ([0, 6].includes(new Date(`${record.work_date}T12:00:00+09:00`).getDay())) return "휴일";
  }
  return STATUS_LABEL[record.attendance_status];
};
const attendanceStatusTone = (record: AttendanceRecord, requests: CorrectionRequest[], holidays: Holiday[] = []) => !record.clock_in_at && (approvedLeaveForRecord(record, requests, holidays) || approvedEmergencyMinutesForRecord(record, requests) > 0 || [0, 6].includes(new Date(`${record.work_date}T12:00:00+09:00`).getDay())) ? "positive" : statusTone(record.attendance_status);
const attendanceNeedsAttention = (record: AttendanceRecord, requests: CorrectionRequest[]) => {
...
const attendanceLeaveLabel = (record: AttendanceRecord, requests: CorrectionRequest[], holidays: Holiday[] = []) => {
  const isHoliday = holidays.some((holiday) => holiday.holiday_date === record.work_date);
  const labels = approvedLeavesForRecord(record, requests, holidays).flatMap((request) => {
    const minutes = approvedRequestMinutesForDate(request, record.work_date, isHoliday);
    if (minutes <= 0) return [];
    ...
    return `${request.request_subtype || (request.request_type === "special_leave" ? "특별휴가" : "교육휴가")} ${leaveDayLabel(minutes)}`;
  });
...
const overtimeReasonForRecord = (record: AttendanceRecord, requests: CorrectionRequest[], auditLogs: AuditLog[] = []) => requests
  .filter((request) => request.request_type === "overtime" && request.status === "approved" && request.employee_id === record.employee_id && request.target_date === record.work_date)
  .sort((left, right) => right.requested_at.localeCompare(left.requested_at))[0]?.reason
  || auditLogs.filter((log) => log.attendance_record_id === record.id && log.action_type === "overtime_review")
    .sort((left, right) => right.created_at.localeCompare(left.created_at))[0]?.reason || "";
...
function recordAttendancePlaceLabel(record: AttendanceRecord, action: "clock_in" | "clock_out") {
  const recordedAt = action === "clock_in" ? record.clock_in_at : record.clock_out_at;
  if (!recordedAt) return "미기록";
  return attendancePlaceLabel(action, action === "clock_in" ? record.clock_in_location_status : record.clock_out_location_status, action === "clock_in" ? record.clock_in_ip_matched : record.clock_out_ip_matched);
}
...
export default function AttendanceApp() {
  ...
  const tenantBranding = useMemo(() => organizationBranding(!profile && isSupabaseConfigured ? tenantOrganization || PUBLIC_LOGIN_BRANDING : effectiveRole === "super_admin" ? superAdminBranding : tenantOrganization), [effectiveRole, profile, superAdminBranding, tenantOrganization]);
  ...
  useEffect(() => {
    ...
    const androidInstallTimer = window.setTimeout(() => setAndroidInstallAvailable(androidDevice && !installed), 0);
    window.addEventListener("beforeinstallprompt", captureInstallPrompt);
    return () => { window.clearInterval(timer); window.clearTimeout(iosInstallTimer); window.clearTimeout(androidInstallTimer); window.removeEventListener("beforeinstallprompt", captureInstallPrompt); };
  }, []);

  useEffect(() => {
    if (!isSupabaseConfigured || profile) return;
    let active = true;
    void fetch("/api/organization-context", { cache: "no-store" })
      .then((response) => response.ok ? response.json() : null)
      .then((result: { organization?: TenantOrganization } | null) => {
        if (active && result?.organization) setTenantOrganization(result.organization);
      })
      .catch(() => undefined);
    return () => { active = false; };
  }, [profile]);
  ...
  const myRecords = useMemo(() => includeExceptionDays(records.filter((record) => record.employee_id === currentProfile?.id && record.work_date.startsWith(selectedMonth))).filter((record) => record.employee_id === currentProfile?.id), [records, exceptions, requests, profiles, holidays, currentProfile, selectedMonth]);
  ...
  const allMonthRows = useMemo(() => includeExceptionDays(records.filter((record) => record.work_date.startsWith(selectedMonth))), [records, exceptions, requests, profiles, holidays, selectedMonth]);
  const monthlyRows = useMemo(() => allMonthRows.filter((record) => record.work_date.startsWith(selectedMonth)
    && (employeeFilter === "all" || record.employee_id === employeeFilter)
    && (statusFilter === "all" || record.attendance_status === statusFilter)), [allMonthRows, selectedMonth, employeeFilter, statusFilter]);
  ...
  async function acknowledgeWeeklyOvertimeIfRequired(employeeId: string, workDate: string, proposedMinutes: number, excludeRecordId = "", excludeRequestId = "") {
    ...
    const reason = window.prompt(`${weekly.start}부터 ${weekly.end}까지 승인된 시간외근무와 긴급지원 합계가 ${formatMinutes(weekly.total)}으로 주 12시간을 넘습니다.\n\n초과 승인 사유를 5자 이상 입력해 주세요. 승인자, 합계와 사유가 변경 이력에 남습니다.`) || "";
  ...
  async function cancelCorrectionRequest(request: CorrectionRequest) {
    if (!supabase || !currentProfile) return;
    const reason = window.prompt("신청을 취소하는 이유를 입력해 주세요.") || "";
    if (reason.trim().length < 2) return;
    const { error } = await supabase.rpc("employee_cancel_correction_request", { p_request_id: request.id, p_reason: reason.trim() });
    if (error) { setNotice({ tone: "error", text: "신청을 취소하지 못했습니다. 이미 처리됐는지 확인하고 데이터베이스 보완 SQL을 적용해 주세요." }); return; }
    await loadRemoteData(currentProfile);
    setNotice({ tone: "success", text: "신청을 취소했습니다. 취소 사유는 변경 이력에 남습니다." });
  }
  ...
  async function reopenRequest(request: CorrectionRequest) {
    ...
    if (supabase) {
      ...
      if (error) { const message = String(error.message || ""); const detail = message.includes("MONTH_CLOSED") ? "마감된 달입니다. 월 마감을 해제한 뒤 다시 시도해 주세요." : message.includes("ORGANIZATION_ACCESS_DENIED") ? "현재 기관에 속하지 않은 신청입니다." : message.includes("ADMIN_REQUIRED") ? "기관관리자 권한을 확인해 주세요. 보완 SQL 적용이 필요할 수 있습니다." : message.includes("APPLIED_CLOCK_CORRECTION") ? "이미 반영된 출퇴근 시각 수정은 연결된 근태기록에서 수정해 주세요." : "최신 데이터베이스 보완 SQL 적용 여부를 확인해 주세요."; setNotice({ tone: "error", text: `요청을 재검토 상태로 바꾸지 못했습니다${error.code ? ` (오류코드 ${error.code})` : ""}. ${detail}` }); return; }
      if (currentProfile) await loadRemoteData(currentProfile);
  ...
  async function cancelApprovedEmergencySupport(request: CorrectionRequest) {
    if (!supabase || !currentProfile) return;
    const reason = window.prompt("승인된 긴급지원 기록을 취소하는 이유를 입력해 주세요.") || "";
    if (reason.trim().length < 2) return;
    const { error } = await supabase.rpc("admin_cancel_emergency_support_work", { p_request_id: request.id, p_reason: reason });
    if (error) { setNotice({ tone: "error", text: `긴급지원 승인을 취소하지 못했습니다${error.code ? ` (오류코드 ${error.code})` : ""}. 데이터베이스 보완 SQL 적용 여부를 확인해 주세요.` }); return; }
    await loadRemoteData(currentProfile);
    setNotice({ tone: "success", text: "긴급지원 승인을 취소했습니다. 취소 사유와 기존 승인 내용은 변경 이력에 남습니다." });
  }
  ...
  async function reviewOvertime(record: AttendanceRecord, decision: "approved" | "rejected") {
    ...
    // 긴급지원은 출퇴근 기반 시간외근무와 별도 항목이다. 같은 시간대가 출퇴근 기록에
    // 포함되어 있어도 시간외근무 인정 가능 시간에서 다시 차감하지 않는다.
    const regularRecordedMinutes = recordedMinutes;
    const rawMinutes = Math.max(record.raw_overtime_minutes || 0, recordedMinutes);
    ...
    const recordDay = new Date(`${record.work_date}T12:00:00+09:00`).getDay();
    const isHolidayWork = [0, 6].includes(recordDay) || holidays.some((holiday) => holiday.holiday_date === record.work_date);
    let approvedMinutes = 0;
    let compTimeCreditMinutes = 0;
    let weeklyNotice = "";
    if (decision === "approved") {
      if (approvalLimit < 60) { setNotice({ tone: "info", text: emergencyMinutes > 0 ? `긴급지원 ${formatMinutes(emergencyMinutes)}은 별도로 인정되어 시간외근무 승인 대상이 없습니다.` : "시간외근무 승인 대상이 없습니다." }); return; }
      const input = window.prompt(`승인할 시간외근무 시간을 분 단위로 입력해 주세요. 긴급지원 ${formatMinutes(emergencyMinutes)}은 별도로 표시됩니다. 시간외근무 인정 가능 ${formatMinutes(approvalLimit)} 안에서 승인할 수 있습니다.`, String(approvalLimit));
      if (input === null) return;
      approvedMinutes = Number(input);
      if (!Number.isInteger(approvedMinutes) || approvedMinutes < 60 || approvedMinutes % 30 !== 0 || approvedMinutes > approvalLimit) {
    ...
    setNotice({ tone: weeklyNotice ? "info" : "success", text: weeklyNotice || (decision === "approved" ? `시간외근무 ${formatMinutes(approvedMinutes)}을 승인했습니다.${compTimeCreditMinutes > 0 ? ` 실제 추가근무 ${formatMinutes(compTimeCreditMinutes)}은 대체휴무로 적립했으며 30일 동안 사용할 수 있습니다.` : " 대체휴무는 적립하지 않았습니다."}` : "시간외근무를 반려했습니다.") });
  }
  ...
  function includeExceptionDays(sourceRows: AttendanceRecord[]) {
    ...
    const next = new Date(`${monthStart}T12:00:00+09:00`); next.setMonth(next.getMonth() + 1);
    const monthEnd = KST_DATE.format(next);
    exceptions.filter((item) => !item.cancelled_at && item.start_date < monthEnd && item.end_date >= monthStart).forEach((item) => {
      ...
      while (KST_DATE.format(date) < end) {
        ...
        if (!keys.has(key)) {
          ...
          const isTraining = item.exception_type === "external_training";
          const exceptionLabel = EXCEPTION_TYPE_LABEL[item.exception_type] || "승인 예외 근무";
          rows.push({ id: `exception-${item.id}-${workDate}`, employee_id: item.employee_id, employee_name: item.employee_name || profiles.find((profile) => profile.id === item.employee_id)?.name, work_date: workDate, work_type: isTrip ? "business_trip" : "approved_other", clock_in_at: null, clock_out_at: null, clock_in_accuracy: null, clock_in_distance: null, clock_in_location_status: "not_checked", clock_out_accuracy: null, clock_out_distance: null, clock_out_location_status: "not_checked", attendance_status: isTrip ? "business_trip" : isTraining ? "education" : "normal", note: `${exceptionLabel}${item.reason ? `: ${item.reason}` : ""}`, is_closed: false });
          keys.add(key);
        }
    ...
    requests.filter((request) => request.status === "approved"
      && ["annual_leave", "comp_time", "sick_leave", "special_leave", "other_leave", "business_trip"].includes(request.request_type)
      && request.target_date < monthEnd
      && (request.end_date || request.target_date) >= monthStart).forEach((request) => {
      const date = new Date(`${request.target_date < monthStart ? monthStart : request.target_date}T12:00:00+09:00`);
      const requestEnd = request.end_date || request.target_date;
      const end = requestEnd >= monthEnd ? monthEnd : KST_DATE.format(new Date(new Date(`${requestEnd}T12:00:00+09:00`).getTime() + 86_400_000));
      while (KST_DATE.format(date) < end) {
        const workDate = KST_DATE.format(date);
        const key = `${request.employee_id}:${workDate}`;
        const weekend = [0, 6].includes(date.getDay());
        const holiday = holidays.some((item) => item.holiday_date === workDate);
        if (!keys.has(key) && ((!weekend && !holiday) || request.request_type === "business_trip")) {
          const isTrip = request.request_type === "business_trip";
          const requestLabel = request.request_subtype || REQUEST_TYPE_LABEL[request.request_type] || "승인 휴가";
          rows.push({ id: `request-${request.id}-${workDate}`, employee_id: request.employee_id, employee_name: request.employee_name || profiles.find((profile) => profile.id === request.employee_id)?.name, work_date: workDate, work_type: isTrip ? "business_trip" : "approved_other", clock_in_at: null, clock_out_at: null, clock_in_accuracy: null, clock_in_distance: null, clock_in_location_status: "not_checked", clock_out_accuracy: null, clock_out_distance: null, clock_out_location_status: "not_checked", attendance_status: isTrip ? "business_trip" : "normal", note: requestLabel, is_closed: false });
          keys.add(key);
        }
        date.setDate(date.getDate() + 1);
      }
  ...
  function exportCsv(rows: AttendanceRecord[]) {
    ...
    const headers = ["직원명", "날짜", "출근시간", "퇴근시간", "출근", "시간외근무", "긴급지원 인정시간", "시간외근무 합계", "대휴", "휴가", "출장", "병가", "비고"];
    const values = rows.map((row) => {
      ...
      const leave = annual ? leaveUnitLabel(Number(annual.requested_value)) : specialLeave ? requestValueLabel(specialLeave) : otherLeave ? requestValueLabel(otherLeave) : "";
      const emergencyMinutes = approvedEmergencyMinutesForRecord(row, requests);
      return [row.employee_name || "", row.work_date, effectiveClockInAt(row, requests) ? formatTime(effectiveClockInAt(row, requests)) : "", effectiveClockOutAt(row, requests) ? formatTime(effectiveClockOutAt(row, requests)) : "", row.clock_in_at ? "1" : "", formatMinutes(regularApprovedOvertimeMinutes(row, requests)), formatMinutes(emergencyMinutes), formatMinutes(totalApprovedOvertimeMinutes(row, requests)), comp ? formatMinutes(Number(comp.requested_value)) : "", leave, row.work_type === "business_trip" || row.attendance_status === "business_trip" ? "1일" : "", row.leave_type === "sick_leave" ? "1일" : "", [attendanceNoteWithoutEmergency(row.note), emergencySupportRemark(row, requests)].filter(Boolean).join("\n")];
    });
  ...
  async function exportExcel(rows: AttendanceRecord[]) {
    ...
    rows.forEach((record) => logSheet.addRow([
      record.employee_name || "", record.work_date, effectiveClockInAt(record, requests) ? formatTime(effectiveClockInAt(record, requests)) : "", recordAttendancePlaceLabel(record, "clock_in"),
      ...
      effectiveClockOutAt(record, requests) ? formatTime(effectiveClockOutAt(record, requests)) : "", recordAttendancePlaceLabel(record, "clock_out"),
      ...
      attendanceStatusLabel(record, requests), record.changed ? "수정됨" : "최초 기록", attendanceNoteWithoutEmergency(record.note),
    ]));
    ...
    employees.forEach((person, employeeIndex) => {
      ...
      for (let day = 1; day <= daysInMonth; day += 1) {
        ...
        const specialLeaveDetail = specialLeaveMinutes > 0 ? `${specialLeave?.request_subtype || "특별휴가"} ${leaveDayLabel(specialLeaveMinutes)} 사용` : "";
        const otherLeaveDetail = otherLeaveMinutes > 0 ? `${otherLeave?.request_subtype || "교육휴가"} ${leaveDayLabel(otherLeaveMinutes)} 사용` : "";
        const sickDetail = sickMinutes > 0 ? `병가 ${Number((sickMinutes / 480).toFixed(3))}일 사용` : "";
        const overtimeDetail = regularOvertimeMinutes > 0 ? `시간외근무 ${formatMinutes(regularOvertimeMinutes)}, 업무내용: ${overtimeReason}` : "";
        ...
        const row = sheet.addRow([day, dayFormatter.format(date), record && effectiveClockInAt(record, requests) ? formatTime(effectiveClockInAt(record, requests)) : "", record && effectiveClockOutAt(record, requests) ? formatTime(effectiveClockOutAt(record, requests)) : "", record?.clock_in_at ? "✓" : "", overtimeHours || "", compHours || "", leaveDayValue || "", trip ? "✓" : "", sickDayValue || "", remarks]);
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page-wrap">
          ...
          {effectiveRole === "super_admin" && adminView === "org_reports" && selectedOrganization && <ReportViewer superAdmin records={monthlyRows} summaryRecords={allMonthRows} requests={requests} exceptions={exceptions} auditLogs={auditLogs} overtimeAfterComp={[]} month={selectedMonth} closing={monthClosing} profiles={profiles} employeeFilter={employeeFilter} statusFilter={statusFilter} onMonth={changeSelectedMonth} onEmployee={setEmployeeFilter} onStatus={setStatusFilter} onCsv={() => exportCsv(monthlyRows)} onExcel={() => void exportExcel(allMonthRows)} onPrint={() => window.print()} />}
          ...
          {!isAdminRole(effectiveRole) && employeeView === "today" && <TodayView now={now} record={todayRecord || openRecord} exception={openRecord ? undefined : todayException} requests={requests} workplace={workplace} busy={busy} locationResult={locationResult} onClock={startClock} />}
          ...
          {!isAdminRole(effectiveRole) && employeeView === "corrections" && <EmployeeCorrections requests={myRequests} activeEmergencyRequest={activeEmergencyRequest} emergencySupportEnabled={organizationSettingsDraft.emergency_support_enabled} onNew={() => setCorrectionOpen(true)} onEmergencyWork={() => setEmployeeEmergencyWorkOpen(true)} onEditRequest={setEditingRequest} onCancelRequest={(request) => void cancelCorrectionRequest(request)} />}
          {!isAdminRole(effectiveRole) && currentProfile?.can_view_reports && employeeView === "team_reports" && <ReportViewer records={monthlyRows} summaryRecords={allMonthRows} requests={requests} exceptions={exceptions} auditLogs={auditLogs} overtimeAfterComp={monthlyOvertimeAfterComp} month={selectedMonth} closing={monthClosing} profiles={profiles} employeeFilter={employeeFilter} statusFilter={statusFilter} onMonth={changeSelectedMonth} onEmployee={setEmployeeFilter} onStatus={setStatusFilter} onCsv={() => exportCsv(monthlyRows)} onExcel={() => void exportExcel(allMonthRows)} onPrint={() => window.print()} />}
          ...
          {isAdminRole(effectiveRole) && adminView === "monthly" && <MonthlyAdmin records={monthlyRows} summaryRecords={allMonthRows} requests={requests} exceptions={exceptions} holidays={holidays} auditLogs={auditLogs} overtimeAfterComp={monthlyOvertimeAfterComp} month={selectedMonth} closing={monthClosing} canReopen={effectiveRole === "super_admin"} profiles={profiles} employeeFilter={employeeFilter} statusFilter={statusFilter} onMonth={changeSelectedMonth} onEmployee={setEmployeeFilter} onStatus={setStatusFilter} onCsv={() => exportCsv(monthlyRows)} onExcel={() => void exportExcel(allMonthRows)} onPrint={() => window.print()} onClose={closeMonth} onReopen={() => void reopenMonth()} onCreate={() => setAttendanceCreateOpen(true)} onEdit={setEditingRecord} onDelete={(record) => void deleteAttendanceRecord(record)} onOvertime={(record, decision) => void reviewOvertime(record, decision)} onOvertimeReopen={(record) => void reopenOvertimeReview(record)} onConfirm={(record) => void confirmAttendanceRecord(record)} onLeave={setLeaveApplyingRecord} />}
          {isAdminRole(effectiveRole) && adminView === "leave_balances" && <LeaveBalanceAdmin profiles={profiles} requests={requests} annualBalances={annualLeaveBalances} compBalances={compTimeBalances} credits={compTimeCredits} onSaveAnnual={(form) => void saveAnnualLeaveEntitlement(form)} onDeleteAnnual={(entitlement) => void deleteAnnualLeaveEntitlement(entitlement)} onSaveComp={(form) => void saveCompTimeCredit(form)} onDeleteComp={(credit) => void deleteCompTimeCredit(credit)} onExtend={(credit) => void extendCompTimeCredit(credit)} />}
...
function TodayView({ now, record, exception, requests, workplace, busy, locationResult, onClock }: { now: Date; record?: AttendanceRecord; exception?: AttendanceException; requests: CorrectionRequest[]; workplace: Workplace; busy: boolean; locationResult: LocationResult | null; onClock: (action: "clock_in" | "clock_out") => void }) {
  const status = exception ? EXCEPTION_TYPE_LABEL[exception.exception_type] || "승인 예외 근무" : record ? attendanceStatusLabel(record, requests) : "출근 전";
...
function EmployeeRecords({ records, requests, exceptions, compTimeBalance: compTimeBalanceRow, annualLeaveBalance, overtimeAfterComp, month, onMonth, onCorrection }: { records: AttendanceRecord[]; requests: CorrectionRequest[]; exceptions: AttendanceException[]; compTimeBalance?: CompTimeBalance; annualLeaveBalance?: AnnualLeaveBalance; overtimeAfterComp?: MonthlyOvertimeAfterComp; month: string; onMonth: (value: string) => void; onCorrection: () => void }) {
  ...
  const standaloneEmergencyRequests = approvedEmergencyRequestsWithoutAttendance(records, requests).filter((request) => request.target_date.startsWith(month));
  return <section><div className="balance-banner"><span>연차 잔액 <strong>{annualLeaveBalance ? `${Number((annualLeaveBalance.remaining_minutes / 480).toFixed(3))}일` : "미등록"}</strong></span><span>이 달 발생분 대휴 사용 <strong>{formatMinutes(overtimeAfterComp?.comp_time_used_from_source_minutes || 0)}</strong></span><span>대휴 제외 시간외 <strong>{formatMinutes(overtimeAfterComp?.overtime_after_comp_minutes || 0)}</strong></span><span>대체휴무 적립 <strong>{formatMinutes(compTimeBalanceRow?.total_granted_comp_time_minutes || compTimeBalanceRow?.approved_overtime_minutes || 0)}</strong></span><span>사용 누계 <strong>{formatMinutes(compTimeBalanceRow?.used_comp_time_minutes || 0)}</strong></span><span>사용 가능 잔액 <strong>{formatMinutes(compTimeBalance)}</strong></span>{compTimeBalanceRow?.next_expiry_on && <span>가장 빠른 만료 <strong>{compTimeBalanceRow.next_expiry_on}</strong></span>}</div><div className="page-heading"><div><span className="kicker">나의 기록</span><h1>월별 근태기록</h1><p>시간외근무와 긴급지원 인정시간을 구분해 표시합니다. 반차, 반반차, 대체휴무도 요청할 수 있습니다.</p></div><button className="secondary-button" onClick={onCorrection}><PencilLine /> 새 근태 신청</button></div><div className="toolbar-card"><MonthPicker value={month} onChange={onMonth} /><div className="summary-inline"><span>출퇴근기록 <strong>{records.length}</strong></span><span>승인 시간외 <strong>{formatMinutes(records.reduce((sum, record) => sum + regularApprovedOvertimeMinutes(record, requests), 0) + requests.filter((request) => request.request_type === "emergency_support" && request.status === "approved" && request.target_date.startsWith(month)).reduce((sum, request) => sum + (request.approved_minutes || request.calculated_minutes || 0), 0))}</strong></span><span>대체휴무 잔액 <strong>{formatMinutes(compTimeBalance)}</strong></span><span>확인 필요 <strong>{records.filter((record) => attendanceNeedsAttention(record, requests)).length}</strong></span></div></div>{exceptions.length > 0 && <div className="employee-exceptions">{exceptions.map((item) => <article key={item.id}><FileClock /><div><strong>{EXCEPTION_TYPE_LABEL[item.exception_type] || "승인 예외 근무"}</strong><p>{item.start_date}부터 {item.end_date}까지</p></div><Badge tone="active">출퇴근 기록 예외</Badge></article>)}</div>}{standaloneEmergencyRequests.length > 0 && <div className="approved-leave-panel standalone-emergency-panel"><strong>출퇴근 기록 없는 긴급지원 근무</strong><div>{standaloneEmergencyRequests.map((request) => <span key={request.id}>{requestPeriodLabel(request)}, 승인 {formatMinutes(request.approved_minutes || request.calculated_minutes || 0)}</span>)}</div></div>}<div className="mobile-record-list">{records.map((record) => { const emergencyMinutes = approvedEmergencyMinutesForRecord(record, requests); return <article className="record-card" key={record.id}><div className="record-date"><strong>{Number(record.work_date.slice(8, 10))}</strong><span>{new Intl.DateTimeFormat("ko-KR", { weekday: "short", timeZone: "Asia/Seoul" }).format(new Date(`${record.work_date}T12:00:00+09:00`))}</span></div><div className="record-main"><div><strong>출퇴근 기록</strong><Badge tone={attendanceStatusTone(record, requests)}>{attendanceStatusLabel(record, requests)}</Badge></div><p>{formatTime(effectiveClockInAt(record, requests))} <ArrowRight size={14} /> {formatTime(effectiveClockOutAt(record, requests))}</p><small><LogIn size={13} /> {recordAttendancePlaceLabel(record, "clock_in")}</small><small><LogOut size={13} /> {recordAttendancePlaceLabel(record, "clock_out")}</small>{((record.recorded_overtime_minutes || 0) > 0 || emergencyMinutes > 0) && <small><Clock3 size={13} /> {overtimeBreakdownLabel(regularApprovedOvertimeMinutes(record, requests), emergencyMinutes)}</small>}{emergencyMinutes > 0 && <small>{emergencySupportRemark(record, requests)}</small>}{attendanceLeaveLabel(record, requests) && <small><CalendarDays size={13} /> {attendanceLeaveLabel(record, requests)}</small>}</div></article>; })}{records.length === 0 && standaloneEmergencyRequests.length === 0 && exceptions.length === 0 && <EmptyState title="기록이 없습니다" text="선택한 달의 출퇴근 또는 긴급지원 기록이 아직 없습니다." />}</div></section>;
}

function EmployeeCorrections({ requests, activeEmergencyRequest, emergencySupportEnabled, onNew, onEmergencyWork, onEditRequest, onCancelRequest }: { requests: CorrectionRequest[]; activeEmergencyRequest?: CorrectionRequest; emergencySupportEnabled: boolean; onNew: () => void; onEmergencyWork: () => void; onEditRequest: (request: CorrectionRequest) => void; onCancelRequest: (request: CorrectionRequest) => void }) {
...
function AdminDashboard({ records, requests, profiles, onMonthly, onRequests, onConfirm }: { records: AttendanceRecord[]; requests: CorrectionRequest[]; profiles: Profile[]; onMonthly: () => void; onRequests: () => void; onConfirm: (record: AttendanceRecord) => void }) {
  ...
  const attention = records.filter((record) => attendanceNeedsAttention(record, requests)).slice(0, 6);
  return <section><div className="page-heading"><div><span className="kicker">관리자 대시보드</span><h1>오늘의 근태 현황</h1><p>{formatDate(new Date())} 기준, 확인이 필요한 기록을 먼저 보여줍니다.</p></div><button className="secondary-button" onClick={onMonthly}><CalendarDays /> 월별 근태 보기</button></div><div className="metric-grid">{cards.map((card) => <article className={`metric-card metric-${card.tone}`} key={card.label}><div><span>{card.label}</span><strong>{card.value}<small>건</small></strong><p>{card.note}</p></div><card.icon /></article>)}</div><div className="dashboard-grid"><article className="surface-card attention-card"><div className="card-heading"><div><span className="kicker">우선 확인</span><h2>확인이 필요한 기록</h2></div><button onClick={onMonthly}>전체 보기 <ArrowRight /></button></div>{attention.map((record) => <div className="attention-row" key={record.id}><div className="avatar small">{record.employee_name?.slice(0, 1)}</div><div><strong>{record.employee_name}</strong><span>{record.work_date}, {recordAttendancePlaceLabel(record, "clock_in")}</span></div><Badge tone={attendanceStatusTone(record, requests)}>{attendanceStatusLabel(record, requests)}</Badge>{["admin_review", "location_review", "field", "education"].includes(record.attendance_status) && <button className="attention-confirm" onClick={() => onConfirm(record)}><Check size={15} /> 확인 완료</button>}</div>)}{attention.length === 0 && <EmptyState title="확인할 기록이 없습니다" text="현재 별도 확인이 필요한 근태기록이 없습니다." />}</article><article className="surface-card quick-card"><div className="card-heading"><div><span className="kicker">빠른 업무</span><h2>관리 바로가기</h2></div></div><button onClick={onRequests}><span><PencilLine /></span><div><strong>근태 신청 검토</strong><small>{requests.filter((r) => r.status === "pending").length}건 대기 중</small></div><ArrowRight /></button><button onClick={onMonthly}><span><Download /></span><div><strong>근태자료 내보내기</strong><small>CSV, 엑셀, 인쇄</small></div><ArrowRight /></button></article></div></section>;
}

function ReportViewer({ superAdmin = false, records, summaryRecords, requests, exceptions, auditLogs, overtimeAfterComp, month, closing, profiles, employeeFilter, statusFilter, onMonth, onEmployee, onStatus, onCsv, onExcel, onPrint }: { superAdmin?: boolean; records: AttendanceRecord[]; summaryRecords: AttendanceRecord[]; requests: CorrectionRequest[]; exceptions: AttendanceException[]; auditLogs: AuditLog[]; overtimeAfterComp: MonthlyOvertimeAfterComp[]; month: string; closing: MonthClosing | null; profiles: Profile[]; employeeFilter: string; statusFilter: string; onMonth: (value: string) => void; onEmployee: (value: string) => void; onStatus: (value: string) => void; onCsv: () => void; onExcel: () => void; onPrint: () => void }) {
...
function LeaveBalanceAdmin({ profiles, requests, annualBalances, compBalances, credits, onSaveAnnual, onDeleteAnnual, onSaveComp, onDeleteComp, onExtend }: { profiles: Profile[]; requests: CorrectionRequest[]; annualBalances: AnnualLeaveBalance[]; compBalances: CompTimeBalance[]; credits: CompTimeCredit[]; onSaveAnnual: (form: HTMLFormElement) => void; onDeleteAnnual: (entitlement: AnnualLeaveBalance) => void; onSaveComp: (form: HTMLFormElement) => void; onDeleteComp: (credit: CompTimeCredit) => void; onExtend: (credit: CompTimeCredit) => void }) {
  ...
  const approvedLeaveRequests = requests.filter((request) => request.status === "approved" && ["annual_leave", "comp_time", "sick_leave", "special_leave", "other_leave"].includes(request.request_type)).sort((left, right) => left.target_date.localeCompare(right.target_date) || (left.employee_name || "").localeCompare(right.employee_name || "", "ko-KR") || left.request_type.localeCompare(right.request_type));
  ...
  {section === "summary" && <div className="table-card"><div className="section-heading"><div><span className="kicker">승인 내역</span><h2>승인된 휴가와 대체휴무</h2><p className="card-description">사용 시작일이 빠른 순으로 정렬하며, 같은 날은 직원명 순으로 표시합니다. 같은 내용은 월별 근태기록에도 날짜별로 표시됩니다.</p></div><Badge tone="neutral">총 {approvedLeaveRequests.length}건</Badge></div><div className="table-scroll"><table><thead><tr><th>직원</th><th>구분</th><th>사용 기간</th><th>사용량</th><th>사유</th><th>승인 정보</th></tr></thead><tbody>{approvedLeaveRequests.map((request) => <tr key={request.id}><td><strong>{request.employee_name || employeeName(request.employee_id)}</strong></td><td>{request.request_subtype || REQUEST_TYPE_LABEL[request.request_type] || "휴가"}</td><td>{requestPeriodLabel(request)}</td><td>{requestValueLabel(request)}</td><td>{request.reason || "사유 미입력"}</td><td>{request.reviewer_name || "관리자"}{request.reviewed_at ? `, ${formatDate(request.reviewed_at)}` : ""}</td></tr>)}{approvedLeaveRequests.length === 0 && <tr><td colSpan={6}>승인된 휴가 또는 대체휴무가 없습니다.</td></tr>}</tbody></table></div></div>}
...
function MonthlyAdmin({ records, summaryRecords, requests, exceptions, holidays, auditLogs, overtimeAfterComp, month, closing, canReopen, profiles, employeeFilter, statusFilter, onMonth, onEmployee, onStatus, onCsv, onExcel, onPrint, onClose, onReopen, onCreate, onEdit, onDelete, onOvertime, onOvertimeReopen, onConfirm, onLeave }: { records: AttendanceRecord[]; summaryRecords: AttendanceRecord[]; requests: CorrectionRequest[]; exceptions: AttendanceException[]; holidays: Holiday[]; auditLogs: AuditLog[]; overtimeAfterComp: MonthlyOvertimeAfterComp[]; month: string; closing: MonthClosing | null; canReopen: boolean; profiles: Profile[]; employeeFilter: string; statusFilter: string; onMonth: (value: string) => void; onEmployee: (value: string) => void; onStatus: (value: string) => void; onCsv: () => void; onExcel: () => void; onPrint: () => void; onClose: () => void; onReopen: () => void; onCreate: () => void; onEdit: (record: AttendanceRecord) => void; onDelete: (record: AttendanceRecord) => void; onOvertime: (record: AttendanceRecord, decision: "approved" | "rejected") => void; onOvertimeReopen: (record: AttendanceRecord) => void; onConfirm: (record: AttendanceRecord) => void; onLeave: (record: AttendanceRecord) => void }) {
  const [recordOrder, setRecordOrder] = useState<"latest" | "oldest">("latest");
  const [recordRange, setRecordRange] = useState<"today" | "week" | "month">("month");
  const [showApprovedLeaves, setShowApprovedLeaves] = useState(false);
  const today = currentDateKey();
  const todayValue = new Date(`${today}T12:00:00+09:00`);
  const weekStartValue = new Date(todayValue); weekStartValue.setDate(todayValue.getDate() - ((todayValue.getDay() + 6) % 7));
  const weekEndValue = new Date(weekStartValue); weekEndValue.setDate(weekStartValue.getDate() + 7);
  const weekStart = KST_DATE.format(weekStartValue);
  const weekEnd = KST_DATE.format(weekEndValue);
  if (recordRange === "today") records = records.filter((record) => record.work_date === today);
  if (recordRange === "week") records = records.filter((record) => record.work_date >= weekStart && record.work_date < weekEnd);
  ...
  const approvedLeaveRequests = requests.filter((request) => request.status === "approved" && ["annual_leave", "comp_time", "sick_leave", "special_leave", "other_leave"].includes(request.request_type) && request.target_date < monthAfterDate && (request.end_date || request.target_date) >= monthStartDate && (employeeFilter === "all" || request.employee_id === employeeFilter))
    .sort((left, right) => left.target_date.localeCompare(right.target_date) || (left.employee_name || "").localeCompare(right.employee_name || "", "ko-KR") || left.request_type.localeCompare(right.request_type));
  const standaloneEmergencyRequests = approvedEmergencyRequestsWithoutAttendance(summaryRecords, requests).filter((request) => request.target_date.startsWith(month) && (employeeFilter === "all" || request.employee_id === employeeFilter));
  return <section className="print-area">
    ...
    <div className="table-card"><div className="table-scroll"><table><thead><tr><th><button type="button" className="date-sort-button" onClick={() => setRecordOrder((current) => current === "latest" ? "oldest" : "latest")} aria-label={recordOrder === "latest" ? "현재 최신 날짜 먼저. 과거 날짜 먼저로 변경" : "현재 과거 날짜 먼저. 최신 날짜 먼저로 변경"}>날짜 {recordOrder === "latest" ? "↓" : "↑"}</button></th><th>직원</th><th>출근</th><th>출근장소</th><th>퇴근</th><th>퇴근장소</th><th>근태상태</th><th>시간외근무</th><th>휴가</th><th>수정 여부</th><th>비고</th><th>관리</th></tr></thead><tbody>{records.map((record) => { const emergencyMinutes = approvedEmergencyMinutesForRecord(record, requests); const regularRecorded = record.recorded_overtime_minutes || 0; const regularApproved = regularApprovedOvertimeMinutes(record, requests); const overtimeReason = overtimeReasonForRecord(record, requests, auditLogs); const isExceptionOnly = record.id.startsWith("exception-") || record.id.startsWith("emergency-") || record.id.startsWith("request-"); return <tr key={record.id}><td>{record.work_date.slice(5).replace("-", ". ")}.</td><td><strong>{record.employee_name}</strong></td><td>{formatTime(effectiveClockInAt(record, requests))}</td><td><span className="location-cell"><LogIn size={15} />{recordAttendancePlaceLabel(record, "clock_in")}{record.clock_in_at && record.clock_in_distance != null && !record.clock_in_ip_matched && <small>{record.clock_in_distance}m</small>}</span></td><td>{formatTime(effectiveClockOutAt(record, requests))}</td><td><span className="location-cell"><LogOut size={15} />{recordAttendancePlaceLabel(record, "clock_out")}{record.clock_out_at && record.clock_out_distance != null && !record.clock_out_ip_matched && <small>{record.clock_out_distance}m</small>}</span></td><td><Badge tone={attendanceStatusTone(record, requests, holidays)}>{attendanceStatusLabel(record, requests, holidays)}</Badge></td><td>{regularRecorded > 0 || emergencyMinutes > 0 ? <div className="record-actions overtime-actions"><span>{overtimeBreakdownLabel(regularRecorded, emergencyMinutes)}{overtimeReason && <small>사유: {overtimeReason}</small>}</span>{record.overtime_status === "approved" || record.overtime_status === "rejected" ? <><Badge tone={record.overtime_status === "approved" ? "positive" : "danger"}>{record.overtime_status === "approved" ? `시간외근무 ${formatMinutes(regularApproved)} 승인` : "시간외근무 반려"}</Badge><button type="button" onClick={() => onOvertimeReopen(record)} disabled={isClosed}><RefreshCw size={15} /> 재검토</button></> : regularRecorded > 0 ? <><button type="button" onClick={() => onOvertime(record, "approved")} disabled={isClosed}><Check size={15} /> 승인</button><button type="button" className="danger" onClick={() => onOvertime(record, "rejected")} disabled={isClosed}><X size={15} /> 반려</button></> : <Badge tone="positive">긴급지원 승인</Badge>}</div> : ""}</td><td>{attendanceLeaveLabel(record, requests, holidays)}</td><td>{record.changed ? "있음" : "없음"}</td><td>{[attendanceNoteWithoutEmergency(record.note), emergencySupportRemark(record, requests)].filter(Boolean).join("\n")}</td><td>{isExceptionOnly ? <Badge tone="active">예외 일정 자동 표시</Badge> : <div className="record-actions"><button type="button" onClick={() => onLeave(record)} disabled={isClosed}><CalendarDays size={15} /> 휴가 반영</button><button type="button" onClick={() => onEdit(record)} disabled={isClosed}><PencilLine size={15} /> 수정</button><button type="button" className="danger" onClick={() => onDelete(record)} disabled={isClosed}><Trash2 size={15} /> 삭제</button></div>}</td></tr>; })}</tbody></table>{records.length === 0 && <EmptyState title="표시할 기록이 없습니다" text="선택한 조건에 맞는 근태기록이 없습니다." />}</div></div>
  </section>;
}
...
function ExceptionAdmin({ exceptions, onNew, onCancel }: { exceptions: AttendanceException[]; onNew: () => void; onCancel: (item: AttendanceException) => void }) {
  ...
  const monthly = exceptions.filter((item) => !item.cancelled_at && item.start_date < nextMonth && item.end_date >= firstDay)
    .sort((left, right) => left.start_date.localeCompare(right.start_date) || (left.employee_name || "").localeCompare(right.employee_name || "", "ko-KR") || left.end_date.localeCompare(right.end_date));
...
function RequestAdminWorkspace({ requests, records, emergencySupportEnabled, onReview, onReopen, onCancelApprovedEmergency, onEditRequest, onEditRecord, onEmergencyWork }: { requests: CorrectionRequest[]; records: AttendanceRecord[]; emergencySupportEnabled: boolean; onReview: (request: CorrectionRequest, decision: "approved" | "rejected" | "more_info") => void; onReopen: (request: CorrectionRequest) => void; onCancelApprovedEmergency: (request: CorrectionRequest) => void; onEditRequest: (request: CorrectionRequest) => void; onEditRecord: (record: AttendanceRecord) => void; onEmergencyWork: () => void }) {
  ...
  return <section>
    ...
    <div className="review-list">{visible.map((request) => {
      ...
      const overtimeLimit = Math.min(240, requestedOvertime);
      const unfinishedEmergency = request.request_type === "emergency_support" && !request.end_time;
      const reviewStatusLabel = unfinishedEmergency && isOpen ? "지원 진행 중" : request.request_type === "overtime" && request.status === "pending" ? "사전 승인 대기" : statusLabels[request.status];
      return <article className="review-card" key={request.id}>
        ...
        {request.request_type === "overtime" && <div className="summary-inline"><span>신청 <strong>{formatMinutes(requestedOvertime)}</strong></span><span>사전 승인 가능 <strong>{formatMinutes(overtimeLimit)}</strong></span><span>실제 인정 <strong>{linkedRecord?.clock_out_at ? formatMinutes(Math.min(overtimeLimit, actualOvertime)) : "퇴근할 때 확정"}</strong></span></div>}
        ...
        <div className="review-actions">{isOpen ? <>
          ...
          <button className="approve-button" onClick={() => onReview(request, "approved")} disabled={unfinishedEmergency}><Check /> {unfinishedEmergency ? "종료 후 승인" : request.request_type === "overtime" ? "사전 승인" : "승인"}</button>
        </> : appliedClockRequest ? linkedRecord && <button className="secondary-button" onClick={() => onEditRecord(linkedRecord)}><PencilLine /> 연결된 근태기록 수정</button> : request.status !== "cancelled" && (request.request_type === "emergency_support" && request.status === "approved" ? <><button className="reject-button" onClick={() => onCancelApprovedEmergency(request)}><X /> 승인 취소</button><button className="secondary-button" onClick={() => { onReopen(request); setStatusView("pending"); }}><RefreshCw /> 재검토</button></> : <><button className="secondary-button" onClick={() => onEditRequest(request)}><PencilLine /> 내용 수정</button><button className="secondary-button" onClick={() => { onReopen(request); setStatusView("pending"); }}><RefreshCw /> 재검토</button></>)}</div>
      </article>;
...
function RequestAdmin({ requests, records, onReview, onReopen, onEditRequest, onEditRecord, onEmergencyWork }: { requests: CorrectionRequest[]; records: AttendanceRecord[]; onReview: (request: CorrectionRequest, decision: "approved" | "rejected" | "more_info") => void; onReopen: (request: CorrectionRequest) => void; onEditRequest: (request: CorrectionRequest) => void; onEditRecord: (record: AttendanceRecord) => void; onEmergencyWork: () => void }) {
  ...
  return <section>
    ...
    <div className="review-list">{visible.map((request) => {
      ...
      const overtimeLimit = Math.min(240, requestedOvertime);
      const unfinishedEmergency = request.request_type === "emergency_support" && !request.end_time;
      const reviewStatusLabel = unfinishedEmergency ? "지원 진행 중" : request.request_type === "overtime" && request.status === "pending" ? "사전 승인 대기" : statusLabels[request.status];
      return <article className="review-card" key={request.id}><div className="review-person"><div className="avatar">{request.employee_name?.slice(0, 1)}</div><div><strong>{request.employee_name}</strong><span>{requestPeriodLabel(request)}, {REQUEST_TYPE_LABEL[request.request_type] || "근태 신청"}</span></div><Badge tone={unfinishedEmergency ? "active" : statusTone(request.status)}>{reviewStatusLabel}</Badge></div><div className="change-box"><div><span>{["clock_in_at", "clock_out_at"].includes(request.request_type) ? "변경 전" : "기존 기록"}</span><strong>{request.before_value.length > 60 ? "기존 기록 있음" : request.before_value}</strong></div><ArrowRight /><div><span>{["clock_in_at", "clock_out_at"].includes(request.request_type) ? "요청 값" : "신청 내용"}</span><strong>{requestValueLabel(request)}</strong></div></div>{request.request_type === "overtime" && <div className="summary-inline"><span>신청 <strong>{formatMinutes(requestedOvertime)}</strong></span><span>사전 승인 가능 <strong>{formatMinutes(overtimeLimit)}</strong></span><span>실제 인정 <strong>{linkedRecord?.clock_out_at ? formatMinutes(Math.min(overtimeLimit, actualOvertime)) : "퇴근할 때 확정"}</strong></span></div>}<div className="reason-box"><span>신청 사유</span><p>{request.reason}</p></div>{request.reviewed_at && <small>{request.reviewer_name || "관리자"} 처리, {formatDate(request.reviewed_at)} {formatTime(request.reviewed_at)}{request.reviewer_comment ? `, ${request.reviewer_comment}` : ""}</small>}<div className="review-actions">{isOpen ? <><button className="reject-button" onClick={() => onReview(request, "rejected")}><X /> 반려</button>{!unfinishedEmergency && <button className="secondary-button" onClick={() => onEditRequest(request)}><PencilLine /> 내용 수정</button>}<button className="secondary-button" onClick={() => onReview(request, "more_info")}><AlertCircle /> 추가정보 요청</button><button className="approve-button" onClick={() => onReview(request, "approved")} disabled={unfinishedEmergency}><Check /> {unfinishedEmergency ? "종료 후 승인" : request.request_type === "overtime" ? "사전 승인" : "승인"}</button></> : appliedClockRequest ? linkedRecord && <button className="secondary-button" onClick={() => onEditRecord(linkedRecord)}><PencilLine /> 연결된 근태기록 수정</button> : <><button className="secondary-button" onClick={() => onEditRequest(request)}><PencilLine /> 내용 수정</button><button className="secondary-button" onClick={() => onReopen(request)}><RefreshCw /> 재검토</button></>}</div></article>;
    })}{visible.length === 0 && <EmptyState title="표시할 요청이 없습니다" text="선택한 달과 처리상태에 해당하는 요청이 없습니다." />}</div>
  </section>;
}
...
function AttendanceRequestEditModal({ request, canChangeType, onClose, onSubmit }: { request: CorrectionRequest; canChangeType: boolean; onClose: () => void; onSubmit: (form: HTMLFormElement) => void }) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="request-edit-title">
    <form className="modal correction-modal" onSubmit={(event) => { event.preventDefault(); onSubmit(event.currentTarget); }}>
      ...
      <div className="form-grid">
        {canChangeType && !isEmergency ? <label className="full">요청 항목<select name="request_type" value={requestType} onChange={(event) => setRequestType(event.target.value)} required><option value="clock_in_at">출근시각 수정</option><option value="clock_out_at">퇴근시각 수정</option><option value="annual_leave">연차 사용</option><option value="comp_time">대체휴무 사용</option><option value="sick_leave">병가</option><option value="special_leave">특별휴가</option><option value="business_trip">출장</option><option value="overtime">시간외근무</option><option value="other_leave">교육휴가</option></select></label> : <input name="request_type" type="hidden" value={requestType} />}
        ...
        {["special_leave", "other_leave"].includes(requestType) && <label className="full">{requestType === "special_leave" ? "특별휴가 종류" : "교육휴가명"}<input name="request_subtype" defaultValue={request.request_subtype || ""} placeholder={leaveSubtypePlaceholder(requestType)} minLength={2} required /></label>}
