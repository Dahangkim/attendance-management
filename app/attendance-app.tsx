...
const DEFAULT_ORGANIZATION_SETTINGS: OrganizationSettings = { id: true, timezone: "Asia/Seoul", default_start_time: "09:00", default_end_time: "18:00", break_minutes: 60, late_grace_minutes: 0, early_leave_grace_minutes: 0, office_ip_address: "" };
...
const normalizeOrganizationSettings = (value: OrganizationSettings): OrganizationSettings => ({ ...value, office_ip_address: value.office_ip_address || "", default_start_time: value.default_start_time.slice(0, 5), default_end_time: value.default_end_time.slice(0, 5) });
...
export default function AttendanceApp() {
  ...
  async function saveSettings() {
    if (settingsDraft.allowed_radius_meters < 50 || settingsDraft.allowed_radius_meters > 1000) { setNotice({ tone: "warning", text: "허용 반경은 50m에서 1,000m 사이로 입력해 주세요." }); return; }
  ...
  async function persistWorkplace(next: Workplace): Promise<{ workplace: Workplace | null; errorCode: string | null }> {
    if (!supabase) return { workplace: next, errorCode: null };
    const { data, error } = await supabase.rpc("save_workplace_settings", {
      p_workplace_name: next.workplace_name,
      p_latitude: next.latitude,
      p_longitude: next.longitude,
      p_allowed_radius_meters: next.allowed_radius_meters,
      p_low_accuracy_threshold_meters: next.low_accuracy_threshold_meters,
    });
    if (error) return { workplace: null, errorCode: error.code || "RPC_ERROR" };
    const saved = (Array.isArray(data) ? data[0] : data) as Workplace | null;
    return { workplace: saved || next, errorCode: null };
  }
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${menuOpen ? "sidebar-open" : ""}`}>
        ...
        {isAdminRole(effectiveRole) && <SupportContact compact />}
      </aside>
      <main className="main-content">
        ...
        <div className="page-wrap">
          ...
          {!isAdminRole(effectiveRole) && employeeView === "corrections" && <EmployeeCorrections requests={myRequests} activeEmergencyRequest={activeEmergencyRequest} onNew={() => setCorrectionOpen(true)} onEmergencyWork={() => setEmployeeEmergencyWorkOpen(true)} onEditRequest={setEditingRequest} />}
          ...
          {isAdminRole(effectiveRole) && adminView === "requests" && <RequestAdminWorkspace requests={requests} records={records} onReview={reviewRequest} onReopen={(request) => void reopenRequest(request)} onEditRequest={setEditingRequest} onEditRecord={setEditingRecord} onEmergencyWork={() => setEmergencyWorkOpen(true)} />}
          {isAdminRole(effectiveRole) && adminView === "audit" && <AuditView logs={auditLogs} brandTitle={tenantBranding.title} onRestore={(log) => void restoreAttendanceRecord(log)} superAdmin={effectiveRole === "super_admin"} selectedOrgId={selectedOrgId} organizations={organizations} organizationChanges={organizationChangeRequests} />}
      ...
      {emergencyWorkOpen && <EmergencySupportWorkModal profiles={profiles} busy={busy} onClose={() => setEmergencyWorkOpen(false)} onSubmit={(form) => void createEmergencySupportWork(form)} />}
      {employeeEmergencyWorkOpen && <EmployeeEmergencySupportControlModal activeRequest={activeEmergencyRequest} busy={busy} onClose={() => setEmployeeEmergencyWorkOpen(false)} onPastSubmit={(form) => void submitCorrection(form)} onStart={(form) => void startEmergencySupportWork(form)} onFinish={(form, request) => void finishEmergencySupportWork(form, request)} onEdit={(request) => { setEmployeeEmergencyWorkOpen(false); setEditingRequest(request); }} onCancel={(request) => void cancelEmergencySupportWork(request)} />}
      ...
      {passwordOpen && currentProfile && <PasswordChangeModal recovery={passwordRecovery} busy={busy} onClose={() => { if (!passwordRecovery) setPasswordOpen(false); }} onSubmit={changeOwnPassword} />}
...
function EmployeeCorrections({ requests, activeEmergencyRequest, onNew, onEmergencyWork, onEditRequest }: { requests: CorrectionRequest[]; activeEmergencyRequest?: CorrectionRequest; onNew: () => void; onEmergencyWork: () => void; onEditRequest: (request: CorrectionRequest) => void }) {
