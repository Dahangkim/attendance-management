...
const requestValueLabel = (request: CorrectionRequest) => {
  ...
  if (["special_leave", "other_leave"].includes(request.request_type)) return `${request.request_subtype || (request.request_type === "special_leave" ? "특별휴가" : "기타 휴가")}, ${leaveDayLabel(minutes)}`;
...
const attendanceLeaveLabel = (record: AttendanceRecord, requests: CorrectionRequest[], holidays: Holiday[] = []) => {
  ...
  const labels = approvedLeavesForRecord(record, requests, holidays).flatMap((request) => {
    ...
    return `${request.request_subtype || (request.request_type === "special_leave" ? "특별휴가" : "기타 휴가")} ${leaveDayLabel(minutes)}`;
  });
  ...
  async function exportExcel(rows: AttendanceRecord[]) {
    ...
    employees.forEach((person, employeeIndex) => {
      ...
      for (let day = 1; day <= daysInMonth; day += 1) {
        ...
        const otherLeaveDetail = otherLeaveMinutes > 0 ? `${otherLeave?.request_subtype || "기타 휴가"} ${leaveDayLabel(otherLeaveMinutes)} 사용` : "";
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="request-edit-title">
    <form className="modal correction-modal" onSubmit={(event) => { event.preventDefault(); onSubmit(event.currentTarget); }}>
      ...
      <div className="form-grid">
        ...
        {["special_leave", "other_leave"].includes(requestType) && <label className="full">{requestType === "special_leave" ? "특별휴가 종류" : "기타 휴가명"}<input name="request_subtype" defaultValue={request.request_subtype || ""} placeholder={leaveSubtypePlaceholder(requestType)} minLength={2} required /></label>}
