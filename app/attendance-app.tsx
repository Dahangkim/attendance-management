...
export default function AttendanceApp() {
  ...
  const myRecords = useMemo(() => includeExceptionDays(records.filter((record) => record.employee_id === currentProfile?.id && record.work_date.startsWith(selectedMonth))).filter((record) => record.employee_id === currentProfile?.id), [records, exceptions, requests, profiles, currentProfile, selectedMonth]);
  ...
  const allMonthRows = useMemo(() => includeExceptionDays(records.filter((record) => record.work_date.startsWith(selectedMonth))), [records, exceptions, requests, profiles, selectedMonth]);
